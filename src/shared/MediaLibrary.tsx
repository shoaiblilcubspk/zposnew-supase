...
import { listMedia, deleteMedia, type MediaAsset } from '../lib/services/mediaService';
...
export function MediaLibrary({ isOpen, onClose, onSelect, standalone }: MediaLibraryProps) {
  ...
  const refreshMedia = useCallback(async () => {
    setMediaAssets(await listMedia());
  }, []);

  // Load the reusable Media Library (uploads + Pexels picks) whenever the picker opens.
  useEffect(() => {
    if (isOpen) void refreshMedia();
  }, [isOpen, refreshMedia]);
  ...
  const handleDeleteImage = async (e: React.MouseEvent, asset: { image: string; mediaId?: string }) => {
    e.stopPropagation();
    ...
    const { isConfirmed } = await sonner.confirm(
      'Delete Asset?',
      asset.mediaId
        ? 'Remove this image from your Media Library? Products already using it keep their copy.'
        : 'This image will be removed from ALL products currently using it. Are you sure?'
    );
    if (!isConfirmed) return;

    // Media Library asset (upload / Pexels pick): soft-delete the media_assets row (syncs everywhere).
    if (asset.mediaId) {
      try {
        await deleteMedia(asset.mediaId);
        await refreshMedia();
        sonner.success('Removed from Media Library.');
      } catch (error) {
  ...
  // Combined gallery: reusable Media Library (uploads + Pexels picks) FIRST, then every image
  // already used by products/bundles/logo. Deduped by image value so the same picture appears once.
  const productAssets = useMemo(() => {
    const uniqueImages = new Map<string, any>();

    // Media Library assets (uploads + Pexels picks) — reusable across all devices via the
    // saved link/hash; these show even before being attached to any product.
    mediaAssets.forEach((m) => {
      const value = m.imageHash || m.srcUrls.large || m.srcUrls.large2x || m.srcUrls.medium || '';
      if (!value || uniqueImages.has(value)) return;
      uniqueImages.set(value, {
        id: `media-${m.id}`,
        mediaId: m.id,
        name: m.source === 'pexels' ? (m.photographer ? `Pexels · ${m.photographer}` : 'Pexels') : 'Uploaded',
        sku: m.source === 'pexels' ? 'PEXELS' : 'UPLOAD',
        image: value,
        isSystem: false,
      });
    });
    ...
    if (appSettings?.storeLogo && (appSettings.storeLogo.startsWith('http') || appSettings.storeLogo.startsWith('data:image'))) {
      if (!uniqueImages.has(appSettings.storeLogo)) {
  ...
  }, [appProducts, appSettings?.storeLogo, appBundles, mediaAssets]);
    ...
    <div className={cn("p-0 custom-scrollbar", standalone ? "h-full" : "")}>
      {standalone && (
        <div className="p-4 border-b border-neutral-200 dark:border-white/[0.08] bg-neutral-50 dark:bg-white/[0.02]">
          ...
          <p className="text-[12px] text-neutral-500">Uploads, Pexels picks and all product & bundle images — reusable on every device.</p>
        </div>
      )}
